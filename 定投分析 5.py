...
def Regular_Investment_with_auto_sell(t_list, TAKE_PROFIT_RATE=0.50, SELL_RATIO=0.2, gamma=0.9):
    cost2 = 0
    asset2 = 0
    share2 = 0
    ...
    for i, t in enumerate(t_list):
        ...
        # 收益函数（指数滑动平均资产）
        reward = reward*gamma + asset2/cost2  #  / (cost2_in_system+1e-6)
    
    return TIMES2, COST2, ASSETS2, SHARE2, reward

if __name__=='__main__':
    # 网格搜索参数
    TAKE_PROFIT_RATE_range = np.arange(0, 1.51, 0.1)  # 累计收益率阈值
    SELL_RATIO_range = np.arange(0, 1.01, 0.1)  # 卖出比例（保持原值）

    results = []
    for take_profit in TAKE_PROFIT_RATE_range:
        for sell_ratio in SELL_RATIO_range:
            print("止盈点：", take_profit, "止盈比例：", sell_ratio)
            _, _, _, _, reward = Regular_Investment_with_auto_sell(
                t_list, TAKE_PROFIT_RATE=take_profit, SELL_RATIO=sell_ratio, gamma=0.9
            )
            results.append((take_profit, sell_ratio, reward))
            print("EMA收益：", reward, "\n")

    results = np.array(results)
    # 找到 reward 最大的参数组合
    best_idx = np.argmax(results[:, 2])
    best_take_profit, best_sell_ratio, best_reward = results[best_idx]

    # 绘制参数-收益界面
    fig1 = plt.figure(figsize=(8, 6))
    ax = fig1.add_subplot(111, projection='3d')
    ax.plot_trisurf(results[:, 0], results[:, 1], results[:, 2], cmap='viridis', alpha=0.8)
    ax.scatter([best_take_profit], [best_sell_ratio], [best_reward], color='r', s=60, label='最优点')
    ax.set_xlabel('止盈比例 TAKE_PROFIT_RATE')
    ax.set_ylabel('卖出比例 SELL_RATIO')
    ax.set_zlabel('reward')
    ax.set_title('参数-收益界面')
    ax.legend()

    print("\n")
    print("最佳止盈点", best_take_profit, "最佳卖出比:", best_sell_ratio)

    # 用最优参数再跑一次，获得曲线
    TIMES2, COST2, ASSETS2, SHARE2, reward = Regular_Investment_with_auto_sell(
        t_list, TAKE_PROFIT_RATE=best_take_profit, SELL_RATIO=best_sell_ratio, gamma=0.9
    )

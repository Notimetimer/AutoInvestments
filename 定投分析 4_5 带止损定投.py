...
# 止损规则
STOP_LOSS_RATE = 0.05  # 止损阈值
SELL_RATIO = 0.15        # 卖出比例，不能超过1的
...
plt.plot(TIMES2, ASSETS2, 'b', label='带止损')
plt.title("净资产 (累计收益额)")
plt.legend()
plt.grid()
...
plt.plot(TIMES2, ASSETS2/(COST2+1e-8), 'b', label='带止损')
...
plt.plot(TIMES2, SHARE2, 'b', label='带止损')
plt.title("持有份额")
plt.legend()
plt.grid()
